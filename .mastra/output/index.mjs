...
import { acquireMcp } from './tools/5b3da1ef-802a-464a-906b-cdf0f702773f.mjs';
import { n8nProTools } from './tools/b39f2952-1c23-47c9-abae-d8b7db876bd5.mjs';
...
- FIX all errors before proceeding`,
  ...
  model: async ({ runtimeContext }) => {
    const provider = runtimeContext?.get("provider_llm");
    const apiKey = runtimeContext?.get("api_key_llm");
    const modelName = runtimeContext?.get("model_llm");
    ...
    switch (providerStr) {
      ...
      default: {
        const customOpenAI = createOpenAI({ apiKey: String(apiKey) });
        return customOpenAI(modelName || "gpt-4.1");
      }
    }
  },
  // Динамические инструменты: MCP + локальные HTTP-инструменты n8n admin
  tools: async ({ runtimeContext }) => {
    const role = String(runtimeContext?.get("role") || "").toLowerCase();
    const proUrl = String(runtimeContext?.get("url_by_type") || "");
    const proKey = String(runtimeContext?.get("api_key_by_type") || "");
...
`
  ...
  defaultStreamOptions: async ({ runtimeContext }) => ({
    maxSteps: 20,
    onFinish: async () => {
      const dispose = runtimeContext?.get("mcp_dispose");
      if (typeof dispose === "function") await dispose();
    }
  }),
  defaultVNextStreamOptions: async ({ runtimeContext }) => ({
    maxSteps: 20,
    onFinish: async () => {
      const dispose = runtimeContext?.get("mcp_dispose");
      if (typeof dispose === "function") await dispose();
